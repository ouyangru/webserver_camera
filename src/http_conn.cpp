...
void http_conn::close_conn() {
    if(m_sockfd != -1) {
        if (m_conn_type == CONN_MJPEG && m_stream_manager) {
...
bool http_conn::enqueue_packet(const std::shared_ptr<std::vector<unsigned char>>& packet, size_t max_queue_depth) {
    ...
    bool dropped = false;
    pthread_mutex_lock(&m_send_lock);
    while (m_send_queue.size() >= max_queue_depth && !m_send_queue.empty()) {
...
bool http_conn::build_status_response() {
    ...
    size_t mjpeg_clients = m_stream_manager->mjpeg_client_count();
    size_t dropped = m_stream_manager->dropped_packets();
    size_t enqueued = m_stream_manager->enqueued_packets();
    ...
    snprintf(buf, sizeof(buf),
             "{\"http_clients\":%d,\"mjpeg_clients\":%zu,"
             "\"frame_bytes\":%zu,\"pixel_format\":%d,"
             "\"mjpeg_enqueued\":%zu,\"mjpeg_dropped\":%zu}\n",
             m_user_count, mjpeg_clients, frame_len, pixel_format, enqueued, dropped);
    body.assign(buf);
